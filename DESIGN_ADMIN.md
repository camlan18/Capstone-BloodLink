...
description: "A near-black product-focused marketing canvas built around #010102 (the deepest dark surface of any tool in this collection), light gray text (#f7f8f8), and a signature Blood Red (#e63946) used as the single chromatic accent. The system reads as software-craft documentation: dense, technical, and quietly luxurious. Display type is set in the Linear custom sans (SF Pro Display fallback) at 500–700 with measured negative tracking. Cards live as charcoal panels (#0f1011) with hairline borders. The accent red appears on the brand mark, focus rings, and a few intentional CTAs — never decoratively. Page rhythm leans on product UI screenshots framed in dark panels rather than atmospheric color."
...
The single chromatic accent is **Blood Red** `{colors.primary}` (#e63946) — used on the brand mark, focus rings, and the primary CTA button. A lighter hover state (`{colors.primary-hover}` #ff4d4d) and a focus-tinted variant (`{colors.primary-focus}` #cc0000) extend the same hue. Linear avoids saturated greens, oranges, purples, etc. on the marketing canvas — the only semantic color is `{colors.semantic-success}` (#27a644) for status pills and the rare success indicator.
...
- **Blood Red brand accent** (`{colors.primary}` #e63946) — used scarcely on brand mark, focus, and the primary CTA.
...
### Brand & Accent
- **Blood Red** ({colors.primary}): The signature accent — primary CTA, brand mark, link emphasis.
- **Red Hover** ({colors.primary-hover}): Lighter red (#ff4d4d) — hovered state of the primary CTA.
- **Red Focus** ({colors.primary-focus}): Focus-ring tint (#cc0000) — focused inputs, focused buttons.
- **Brand Secure** ({colors.brand-secure}): Muted red-gray (#8c6b6b) — used in "Linear Security" surfaces.
...
**`button-primary`** — Red CTA. The default primary CTA across all pages.
...
- Use `{colors.primary}` red ONLY for: brand mark, primary CTA, focus ring, link emphasis.
...
- Don't use red as a section background or card fill.
...
6. Treat red as scarce: brand mark, primary CTA, focus, link emphasis.

...
export default function BlogPage() {
  ...
  const selectedCategory = categories.find(c => c.blog_category_id.toString() === categoryId);
      ...
      <div className="bg-gradient-to-r from-navy to-navy-light text-white py-16">
        <div className="container mx-auto px-4">
          <div className="max-w-3xl mx-auto text-center">
            <p className="text-blood font-semibold text-sm uppercase tracking-wider mb-3">Tin tức & Sự kiện</p>
            <h1 className="text-3xl md:text-5xl font-bold mb-6">Blog Cộng Đồng</h1>
            <p className="text-slate-300 mb-8 text-lg">Cập nhật tin tức mới nhất về các chiến dịch hiến máu và câu chuyện truyền cảm hứng</p>
            ...
            <div className="flex items-center overflow-x-auto w-full md:w-auto pb-2 md:pb-0 scrollbar-hide text-sm">
              ...
              {categories.map(cat => (
                <button 
                  ...
                  onClick={() => { setCategoryId(cat.blog_category_id.toString()); setPage(1); }}
                  className={`whitespace-nowrap px-4 border-l border-slate-200 font-medium transition-colors ${categoryId === cat.blog_category_id.toString() ? 'text-navy font-bold' : 'text-slate-500 hover:text-navy'}`}
                >
                  {cat.category_name}
                </button>
              ))}
        ...
        {loading ? (
          ...
          <>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-10">
              {documents.map((doc: BlogPost) => (
                <Card key={doc.post_id} className="overflow-hidden hover:shadow-lg hover:-translate-y-1 transition-all border-slate-100 rounded-lg group">
          ...
          <div className="text-center py-20 bg-white border border-dashed border-slate-200 rounded-lg">
            ...
            <h3 className="text-lg font-semibold text-navy mb-2">Không tìm thấy bài viết</h3>
            <p className="text-slate-500 max-w-md mx-auto">Chúng tôi không tìm thấy bài viết nào phù hợp với bộ lọc hiện tại. Vui lòng thử thay đổi từ khóa hoặc danh mục.</p>
            <button 

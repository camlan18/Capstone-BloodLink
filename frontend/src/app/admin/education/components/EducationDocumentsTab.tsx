...
export default function EducationDocumentsTab() {
  ...
  const fetchAuxData = async () => {
    try {
      const catRes = await adminEducationService.getCategories();
      if (catRes) setCategories(Array.isArray(catRes.data) ? catRes.data : Array.isArray(catRes) ? catRes : []);
      
      const btRes = await adminMasterDataService.getBloodTypes({ limit: 100 });
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const res = await adminEducationService.getDocuments({ page, limit: pageSize, search: keyword });
      if (res) {
  ...
  const handleOpenCreate = () => {
    setEditingItem(null);
    setFormData({ 
      title: '', slug: '', category_id: categories[0]?.category_id?.toString() || '', blood_type_id: '',
  ...
  const handleDelete = async (item: any) => {
    ...
    try {
      await adminEducationService.deleteDocument(item.document_id);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const payload = {
        ...
        blood_type_id: formData.blood_type_id ? Number(formData.blood_type_id) : undefined,
        is_published: formData.is_published === 'true',
      };

      if (editingItem) {
        await adminEducationService.updateDocument(editingItem.document_id, payload);
        toast.success('Cập nhật tài liệu thành công');
      } else {
        await adminEducationService.createDocument(payload);
      ...
      render: (item) => (
        item.blood_type ? (
          <span className="px-2 py-0.5 bg-red-100 text-red-700 rounded text-xs font-bold border border-red-200">
            {item.blood_type.blood_type_code}
          </span>
        ) : <span className="text-slate-400 italic text-sm">Chung</span>
      )
    },
    {
      key: 'views',
  ...
  return (
    <div className="p-6">
      <DataTable
        ...
        toolbarFilters={
          <Button onClick={handleOpenCreate} className="bg-blood hover:bg-blood-deep text-white shadow-md">
            <Plus className="w-4 h-4 mr-2" /> Soạn tài liệu
          </Button>
        }
      />
      ...
      <BaseModal
        ...
        title={editingItem ? "Chỉnh sửa tài liệu" : "Soạn tài liệu giáo dục"}
        size="5xl"
        hideFooter
      >
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div className="col-span-2">
              <label className="block text-sm font-medium text-slate-700 mb-1">Tiêu đề tài liệu <span className="text-red-500">*</span></label>
              <Input 
                type="text" 
              ...
              <Select value={formData.category_id} onValueChange={v => setFormData({...formData, category_id: v || ''})}>
                <SelectTrigger>
                  <SelectValue placeholder="Chọn danh mục">
                    {categories?.find(c => c.category_id?.toString() === formData.category_id)?.category_name || 'Chọn danh mục'}
                  </SelectValue>
                </SelectTrigger>
                <SelectContent>
                  {(categories || []).map(cat => (
            ...
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-1">Liên kết nhóm máu (Tùy chọn)</label>
              <Select value={formData.blood_type_id} onValueChange={v => setFormData({...formData, blood_type_id: v === 'none' ? '' : (v || '')})}>
                <SelectTrigger>
                  <SelectValue placeholder="Dành cho mọi nhóm máu">
                    {formData.blood_type_id ? bloodTypes?.find(b => b.blood_type_id?.toString() === formData.blood_type_id)?.blood_type_code : 'Tất cả nhóm máu'}
                  </SelectValue>
                </SelectTrigger>
                <SelectContent>
                  <SelectItem value="none">Tất cả nhóm máu</SelectItem>
                  {(bloodTypes || []).map(bt => (
            ...
            <div className="col-span-2">
              <label className="block text-sm font-medium text-slate-700 mb-1">Tóm tắt nội dung</label>
              <Input type="text" value={formData.summary} onChange={e => setFormData({...formData, summary: e.target.value})} placeholder="Viết 1-2 câu tóm tắt..." />
            </div>
              ...
              <Select value={formData.is_published} onValueChange={v => setFormData({...formData, is_published: v || ''})}>
                <SelectTrigger>
                  <SelectValue placeholder="Trạng thái">
                    {formData.is_published === 'true' ? 'Xuất bản' : 'Bản nháp'}
                  </SelectValue>
                </SelectTrigger>
                <SelectContent>
          ...
          <div className="pt-4 flex justify-end gap-3 border-t border-slate-100 mt-6">
            ...
            <Button type="submit" className="bg-blood hover:bg-blood-deep text-white">
              {editingItem ? 'Lưu thay đổi' : 'Đăng tải'}
            </Button>
          </div>
        </form>
      </BaseModal>
          ...
          <div className="space-y-6">
            <div className="border-b border-slate-200 pb-4">
              <div className="flex items-center gap-2 mb-3 text-sm">
                ...
                {detailItem.blood_type && (
                  <span className="bg-red-100 text-red-700 font-bold px-2 py-0.5 rounded border border-red-200 text-[10px] tracking-wider">
                    {detailItem.blood_type.blood_type_code}
                  </span>
                )}
